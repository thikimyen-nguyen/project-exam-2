    ...
    bookedDates.forEach(bookedDate => {
      ...
      for (let date = new Date(startDate); date <= endDate; date.setDate(date.getDate() + 1)) {
        if (date >= startDate && date <= endDate) {
          const index = availableDates.findIndex(availableDate => availableDate.getTime() === date.getTime());
          if (index !== -1) {
...
//       calendarDays.push(<div key={`empty-${i}`} className="p-2 bg-gray-100"></div>);
//     }
...
//         <div
//           key={i}
//           className={`p-2 text-center ${isAvailable ? 'bg-green-200' : 'bg-gray-200'}`}
//         >
//           {i}
//         </div>
//       );
...
const renderCalendar = () => {
    ...
    // Add empty cells for days before the first day of the month
    for (let i = 0; i < startingDay; i++) {
      calendarDays.push(<div key={`empty-${i}`} className="p-2 bg-white"></div>);
    }
  ...
  return (
    <div className="text-center my-4 md:w-1/2 m-auto">
      {renderCalendar()}
    </div>
  );
};
